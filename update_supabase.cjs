...
content = content.replace(/ward_id:\s*(\d+),/g, 'ward_ids: [$1],');
content = content.replace(/ward_id:\s*(\d+)\s*\}/g, 'ward_ids: [$1] }');
...
if(content.includes('contains(col: string, arr: any[])') === false) {
    ...
    content = content.replace(/eq\(col:\s*string,\s*val:\s*any\)\s*\{\s*this\.data\s*=\s*this\.data\.filter\(\(i:\s*any\)\s*=>\s*i\[col\]\s*===\s*val\);\s*return\s*this;\s*\}/g, replacement.trim());
}
...
content = content.replace(reportRegex, (match) => {
    ...
    if (status === 'in_progress' || status === 'resolved') {
        ...
        injections += `\n          in_progress_at: '${inProgDate.toISOString()}',`;
...
content = content.replace(/wards:\s*defaultWards\.find\(w\s*=>\s*w\.id\s*===\s*item\.ward_id\)\s*\|\|\s*\{\s*name:\s*'Unknown Ward'\s*\}/g, "wards: defaultWards.find(w => item.ward_ids && item.ward_ids.includes(w.id)) || { name: 'Unknown Ward' }");
